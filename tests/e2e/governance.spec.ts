...
test('the dossier page carries one lifecycle end to end, and the inspector shows the correction’s lineage', async ({ page }) => {
  ...
  await expect(page.getByTestId('dossier-count')).toContainText('1 dossier asked for by a simulated buyer, 2 releases, version 2 corrects version 1, 21 rows refused');
  ...
  await expect(page.getByTestId('dossier-refusals').locator('tbody tr')).toHaveCount(21);
  ...
  await expect(page.getByTestId('dossier-refusals')).toContainText('release_names_a_superseded_assessment');
  await expect(page.getByTestId('dossier-reassessment')).toHaveAttribute('data-changed', 'false');
  await expect(page.getByTestId('dossier-reassessment')).toContainText('COV-2-DEPENDENCY');
  await expect(page.getByTestId('reassessment-rule')).toContainText('never a substitution');
...
test('the control plane counts every governed act, names every guard, and is accessible with the inspector open', async ({ page }) => {
  ...
  await expect(page.getByTestId('governed-counts')).toContainText('15 proposals through one kernel in two databases, and one refused before it was a row: 10 authorized, 1 revoked, 4 dispatched, 4 reconciled, 1 unresolved; 40 rows refused by 38 named guards');
  ...
  await expect(page.getByTestId('all-refusals').locator('tbody tr')).toHaveCount(40);
