...
import { REGULATORY_AUTHORITY_SEPARATOR, REGULATORY_MAX_AUTHORITY_TEXT_CHARS } from './limits';
...
const instrumentSchema = z.object({
  ...
  authority: safeText(REGULATORY_MAX_AUTHORITY_TEXT_CHARS),
...
export function observationsFromFederalRegister(capture: FederalRegisterCaptureInspection): RegulatoryObservation[] {
  ...
  return capture.observations.documents.map((document) => regulatoryObservationSchema.parse({
    ...
    instrument: {
      ...
      authority: document.agencies.length ? document.agencies.join(REGULATORY_AUTHORITY_SEPARATOR) : 'Office of the Federal Register',
