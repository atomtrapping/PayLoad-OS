...
- **Internal navigation.** The draft's home is a root client provider that lives for the whole browser document, above every route. Since 2026-09-12 the root renders a small host (`NotationDraftHost`) rather than the provider itself: the host is empty on every route until the workspace first asks for the controller, then loads the controller's module (on `/notations` its chunk is already in the page's bundle; only the dynamic import's small facade is fetched) and mounts the provider beside the routes, where it stays. The rail reads a separate status store (`draftStatus.ts`), so the other forty-five routes carry neither the editor nor the kernel client. Navigating to another page and back finds the draft, the selection, the pending commands, their original saved base version and any request in flight exactly where they were, without another read of the kernel; nothing is rebased onto a newer saved version behind the person's back. Navigation is never interrupted: instead the rail marks the Notations link `draft` while unsaved work exists, with the counts in its tooltip, on every page.
