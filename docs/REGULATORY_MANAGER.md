...
This increment makes regulatory observation operational on a real public government API contract. The bounded capture path has been exercised against the live API, including its zero-result response shape. A successful capture establishes only what the mirror returned: collection remains operator-enabled, the source registration is an internal qualification with republication review open, and every resulting state proposal still requires review.
