...
import { FederalRegisterCaptureStore, type FederalRegisterCaptureInspection } from '../acquisition/store';
import { parseFederalRegisterBytes, type FederalRegisterCaptureRequest } from '../acquisition/federalRegister';
import { REGULATORY_AUTHORITY_SEPARATOR, REGULATORY_MAX_AUTHORITY_NAMES,
  REGULATORY_MAX_AUTHORITY_NAME_CHARS, REGULATORY_MAX_AUTHORITY_TEXT_CHARS } from './limits';
import {
...
describe('Federal Register bridge', () => {
  const captureRequest: FederalRegisterCaptureRequest = {
    schema: 'payload.federal-register-capture-request.v1', requestId: 'long-authorities',
    publishedFrom: '2026-09-11', publishedThrough: '2026-09-11', perPage: 1,
  };
  const maximumAgencies = Array.from({ length: REGULATORY_MAX_AUTHORITY_NAMES }, (_, index) =>
    `Synthetic Agency ${String(index).padStart(2, '0')} `.padEnd(REGULATORY_MAX_AUTHORITY_NAME_CHARS, 'x'));
  function sourceBytes(agencies: readonly string[]) {
    return Buffer.from(JSON.stringify({ count: 1, results: [{
      document_number: '2026-19001', type: 'Rule', title: 'Synthetic multi-agency rule',
      publication_date: '2026-09-11', effective_on: '2026-10-01', action: 'Final rule.',
      agencies: agencies.map(name => ({ name })),
      html_url: 'https://www.federalregister.gov/documents/2026/09/11/2026-19001/synthetic-rule', pdf_url: null,
    }] }));
  }

  it('retains and analyzes all 32 maximum-length agencies through capture, reopened evidence and bridge', async () => {
    const directory = mkdtempSync(join(tmpdir(), 'regulatory-authorities-'));
    try {
      const store = new FederalRegisterCaptureStore(directory, {
        fetch: async () => ({ bytes: sourceBytes(maximumAgencies), mediaType: 'application/json', etag: null, lastModified: null }),
        now: () => '2026-09-12T12:00:00.000Z',
      });
      expect((await store.capture(captureRequest, true)).state).toBe('CAPTURED');
      const retained = store.inspect(captureRequest.requestId)!;
      expect(retained.observations!.documents[0].agencies).toEqual(maximumAgencies);
      const observations = observationsFromFederalRegister(retained);
      expect(observations[0].instrument.authority).toBe(maximumAgencies.join(REGULATORY_AUTHORITY_SEPARATOR));
      expect(observations[0].instrument.authority).toHaveLength(REGULATORY_MAX_AUTHORITY_TEXT_CHARS);
      expect(observations[0].entities).toEqual(maximumAgencies);
      const report = analyzeRegulatoryChanges({
        ...request(observations, [{ ...watch, topics: [], entities: [maximumAgencies.at(-1)!] }]),
        knownThrough: '2026-09-12T23:59:59.000Z',
      });
      expect(report.clusters[0].authority).toBe(observations[0].instrument.authority);
      expect(report.clusters[0].entities).toEqual(maximumAgencies);
      expect(report.counts).toMatchObject({ clusters: 1, proposals: watch.targets.length });
      expect(report.canonicalStateMutated).toBe(false);
    } finally {
      expect(dirname(resolve(directory))).toBe(resolve(tmpdir()));
      expect(basename(directory)).toMatch(/^regulatory-authorities-/);
      rmSync(directory, { recursive: true, force: true });
    }
  });

  it('refuses excessive authority text without relaxing source-name or source-count limits', () => {
    const excessive = observation('excessive-authority');
    excessive.instrument.authority = 'a'.repeat(REGULATORY_MAX_AUTHORITY_TEXT_CHARS + 1);
    expect(() => analyzeRegulatoryChanges(request([excessive]))).toThrow();
    expect(() => parseFederalRegisterBytes(sourceBytes([...maximumAgencies, 'Thirty-third agency']), captureRequest)).toThrow('FEDERAL_REGISTER_SCHEMA_MISMATCH');
    expect(() => parseFederalRegisterBytes(sourceBytes(['a'.repeat(REGULATORY_MAX_AUTHORITY_NAME_CHARS + 1)]), captureRequest)).toThrow('FEDERAL_REGISTER_SCHEMA_MISMATCH');
  });
