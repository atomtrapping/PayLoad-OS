...
export const CARAVAN_LOT_STATE: InformationProduct = {
  ...
  fields: [
    ...
    { predicate: 'location.position', meaning: 'Where the lot was over an interval, as a custody system or a party log declares it: a WGS84 point with the horizontal uncertainty the source stated. Never inferred, geocoded or interpolated, and a sample is not placed at its lot.', subjectTypes: ['Lot'], acceptable: { productionClass: ['asserted'], claimStrength: ['reported'], interest: ANY_INTEREST }, note: 'The interest axis is the one thing the Earth Twin colours by: a port operator’s berth and a claimant’s yard are both declarations, and the customer sees whose.' },
