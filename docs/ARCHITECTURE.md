...
[Usage as telemetry](METERING.md) records what a lap is, what a response would have to carry before a bill line could point at it, and the boundary between metering usage and becoming the rails. [Correction and identity](CORRECTION_AND_IDENTITY.md) records the downstream-invalidation machinery, the specified-and-empty delivery ledger, the as-of contract on both sides, and the one identity core with its per-line identifier families and the absent cross-line join. [Storage](STORAGE.md) records the six classes of information, the kind of store each asks for, the candidates, and the doctrine each store must not break; PostgreSQL is selected and wired for records, the other five classes are local files and fixtures. [Estimation](ESTIMATION.md) records the factor graph the estimator, the constraint stack, the cell complex and the measurement economy have all been describing — a constraint is a measurement with `R = 0`, certainty is harvested in proportion to declared confidence, a violated constraint is evidence about the constraint, and a solver informs the corpus but never decides an identity — together with the four tiers of invariant scoring and the reference channel they must never be allowed to feed. [The projection fabric](PROJECTION_FABRIC.md) records the routing table and its two newest targets: OpenUSD as scene interchange — a target and never a store, because USD composition resolves opinions to one value where the corpus preserves disagreement — and the hyperbolic manifold as the tier below the corpus, where void renders void. [Space as a working dimension](SPATIAL_DERIVATION.md) records the first spatial derivation — a cell key no finer than the source's own stated uncertainty, and a metric verdict on whether two positions can be told apart — and the four other jobs space does once it stops being a display attribute. An editable map of every part and its flows, for brainstorming in Obsidian, is in [`docs/architecture-map/`](architecture-map/Payload%20OS%20Architecture.md). [Synthesized architecture](SYNTHESIZED_ARCHITECTURE.md) is the prose of record for the five fabrics, the seven doctrine invariants, the projection instruments, the runtime allocation and the historical concept mapping. This document records only what that architecture binds in this repository: where each rule is enforced, which tests prove it, the three states of information as they exist here, the verification tier reached, and the identity chain. `src/domain/doctrine.ts` holds the same content as data; `/model` renders it; `src/domain/doctrine.test.ts` fails if a named test file disappears.
