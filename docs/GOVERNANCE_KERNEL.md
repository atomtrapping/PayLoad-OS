...
| `dossier_coverage_evidence` | One row per artifact bearing on a facet: artifact, run, assessment, why | `evidence_assessment_is_the_artifacts` recomputes the assessment at commit from the artifact's rights against `required_right`, then a corrected input (`retracted_record`, `CORRECTION`, issued by `assessed_at`), a disagreeing artifact on the same facet about the same subject, or a `FALSIFIED` validation, then a horizon ended by `assessed_at` or a withdrawn input; inputs are followed through derived artifacts to the source records beneath; an artifact computed after `assessed_at` is refused |
...
| `dossier_coverage` again | Re-assessment: one row per facet per instant, keyed `(dossier_facet_id, assessed_at)` | `coverage_assessed_once_per_instant` refuses the same instant twice; `assessment_is_later_than_the_last` refuses a row assessed behind one that already stands. The earlier row is never edited |
| `dossier_release_coverage` | What a release was built over: one assessment per facet | `release_names_its_assessments` (from the release) and `named_assessment_is_the_releases` (from the naming) refuse a release naming no assessment for a facet, two for one facet, another dossier's, one assessed after the build, or one superseded by a later assessment at or before the build — a re-assessment binds every later release |
...
| `dossier_estimate` | Counts the coverage as it stood | `estimate_counts_the_coverage_as_it_stood`: for every facet, the latest assessment at or before `estimated_at`, at its level, by the one method the column may name (`method = 'notationsos.dossier.estimate.v1'`), no facet the dossier did not ask, units the method's |
...
The domain (`src/domain/dossierService.ts`: `assessEvidence`, `rollupAssessment`, `coverageLevel`) computes the same three things in the same order, comparing instants rather than their spellings, and the lifecycle test checks that what it wrote is what the database holds. One hundred and seven tests in `dossierLedger.test.ts`. `npm run mutate:dossier` applies the seventy-two mutations listed in `src/db/dossierLedger.mutations.json` — each guard arm removed, each comparison flipped, each CASE reordered, each written-once trigger dropped — one at a time, runs the file against each, and exits non-zero if any survives; the list is in the tree so the claim is reproducible rather than reported.

Re-assessment is modelled as a new row, not a rewrite: a corpus that moves after the quote is assessed again for the facet at a later instant, the release built after it must name it, and a level that moved is refused under the old scope until an amended quotation is accepted (`dossierLedger.test.ts`, "refuses a release built over a level the quotation did not carry, until an amended quotation is accepted"). Not modelled: what triggers a re-assessment — nothing watches the corpus and asks for one, so a release built after the corpus moved over an un-re-assessed facet is a release over a stale row that the schema cannot tell from a fresh one; and nothing says what a release after an artifact's horizon ends means. The state ledger's `recorded_contradiction` is keyed on operational subjects and has no bridge to artifact subjects, so `CONFLICTING` here comes from the corpus's corrections, from disagreement among artifacts on the facet and from refuted validation, not from that table.
