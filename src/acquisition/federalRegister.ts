...
import { REGULATORY_MAX_AUTHORITY_NAMES, REGULATORY_MAX_AUTHORITY_NAME_CHARS } from '../regulatory/limits';
...
function agencies(value: unknown): string[] | null {
  if (!Array.isArray(value) || value.length > REGULATORY_MAX_AUTHORITY_NAMES) return null;
  const names: string[] = [];
  for (const agency of value) {
    if (!plain(agency)) return null;
    const name = text(agency.name, REGULATORY_MAX_AUTHORITY_NAME_CHARS) ?? text(agency.raw_name, REGULATORY_MAX_AUTHORITY_NAME_CHARS);
    if (!name) return null;
...
function parseDocument(value: unknown, request: FederalRegisterCaptureRequest): FederalRegisterDocumentObservation {
  ...
  if (!htmlUrl || (value.pdf_url !== null && value.pdf_url !== undefined && pdfUrl === null)
    || effectiveOn === undefined || commentsCloseOn === undefined
