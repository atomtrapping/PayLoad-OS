...
| `dossier_coverage`, its evidence, `dossier_release`, `dossier_conclusion` | Written once | `coverage_is_written_once`, `evidence_is_written_once`, `release_is_written_once`, `conclusion_is_written_once` refuse UPDATE and DELETE; `spec_terms_are_written_once` lets a spec move through its stages and nothing else. Every guard above checks a row as it is written; a rewrite would be a row none of them looked at |
...
- **Re-assessment and the state ledger's contradictions** (§8): coverage is assessed once per facet and written once; nothing yet says what a corpus that moved after the quote means for the row, and `recorded_contradiction` does not yet reach an artifact's subject (see the coverage-assessment section).
